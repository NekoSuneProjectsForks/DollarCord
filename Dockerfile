...
# Copy manifests + the Prisma schema first so the `postinstall: prisma generate`
# step has a schema to read during install.
COPY package.json package-lock.json* ./
COPY prisma ./prisma
RUN npm ci || npm install
COPY . .
# Build Next (Prisma client was already generated by postinstall).
RUN npm run build
